  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-lg bg-[var(--bg-card)] border border-[var(--navy)]/50 rounded-sm shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden relative max-h-[90vh]">
        ...
        {/* Tabs */}
        {mode === 'EDIT' && (
            <div className="flex border-b border-white/10 bg-[var(--bg-main)]">
                <button 
                    onClick={() => setActiveTab('DETAILS')}
                    className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-colors ${activeTab === 'DETAILS' ? 'text-white bg-[var(--bg-card)] border-t-2 border-[var(--navy)]' : 'text-neutral-500 hover:bg-white/5'}`}
                >
                    Project Details
                </button>
                <button 
                    onClick={() => setActiveTab('LOGS')}
                    className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-colors ${activeTab === 'LOGS' ? 'text-white bg-[var(--bg-card)] border-t-2 border-emerald-500' : 'text-neutral-500 hover:bg-white/5'}`}
                >
        ...
        {activeTab === 'DETAILS' ? (
            ...
            <div className="mb-4">
                {!showGenerator ? (
                    <button 
                        ...
                        className="w-full py-3 bg-gradient-to-r from-[rgba(0,49,83,0.3)] to-black border border-[var(--navy)]/30 rounded flex items-center justify-center gap-2 text-[var(--navy)] font-bold uppercase text-xs tracking-wider hover:border-[var(--navy)] hover:text-white transition-all group"
                    >
                        <Sparkles size={14} /> Open Genesis Engine
                    </button>
                ) : (
                    <div className="bg-[rgba(0,49,83,0.1)] border border-[var(--navy)]/30 rounded p-4 animate-in slide-in-from-top-2 relative">
                        <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-2 text-[var(--navy)] font-bold text-xs uppercase tracking-widest">
                                <Zap size={14} className="fill-[var(--navy)]" /> Project Genesis
                            </div>
                        ...
                        <textarea 
                            ...
                            className="w-full p-3 bg-black/40 border border-white/10 rounded text-xs text-white placeholder:text-neutral-600 focus:border-[var(--navy)] focus:outline-none resize-none mb-3"
                            rows={2}
                        />
                        <button 
                            type="button"
                            ...
                            className="w-full py-2 bg-[var(--navy)] hover:bg-[var(--navy)] text-white rounded text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors shadow-lg"
                        >
            ...
            <div className="space-y-1.5">
                ...
                <input 
                    ...
                    className={`w-full p-3 bg-[var(--bg-main)] border rounded text-sm text-white focus:outline-none transition-colors font-bold ${errors.name ? 'border-red-500 focus:border-red-500' : 'border-white/10 focus:border-[var(--navy)]'}`}
                />
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                    ...
                    className={`w-full p-3 bg-[var(--bg-main)] border rounded text-sm text-white focus:outline-none transition-colors ${errors.client ? 'border-red-500 focus:border-red-500' : 'border-white/10 focus:border-[var(--navy)]'}`}
                    />
                </div>
                <div className="space-y-1.5">
                    <label className="text-[10px] font-bold uppercase text-neutral-500 tracking-wider flex items-center gap-2">
                    ...
                    className="w-full p-3 bg-[var(--bg-main)] border border-white/10 rounded text-sm text-white focus:border-[var(--navy)] focus:outline-none transition-colors"
                    />
                </div>
            </div>
            ...
            <div className="space-y-1.5">
                ...
                <textarea 
                    ...
                    className="w-full p-3 bg-[var(--bg-main)] border border-white/10 rounded text-sm text-white focus:border-[var(--navy)] focus:outline-none transition-colors resize-none"
                />
            </div>
            ...
            <div className="space-y-1.5">
                ...
                <div className="p-3 bg-[var(--bg-main)] border border-white/10 rounded">
                    <div className="flex flex-wrap gap-2 mb-3">
                        {formData.tags?.map((tag, idx) => (
                            <span key={idx} className="flex items-center gap-1 px-2 py-1 rounded bg-[rgba(0,49,83,0.2)] text-[var(--navy)] text-[10px] font-bold border border-[var(--navy)]/30">
                                {tag}
                    ...
                    <div className="flex gap-2">
                        <input 
                            ...
                            className="flex-1 bg-black/40 border-b border-white/10 text-xs text-white p-1 focus:outline-none focus:border-[var(--navy)] transition-colors"
                        />
                        <button 
                            type="button"
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                    ...
                    className="w-full p-3 bg-[var(--bg-main)] border border-white/10 rounded text-sm text-white focus:border-[var(--navy)] focus:outline-none transition-colors cursor-pointer"
                    >
            ...
            <div className="space-y-2">
                <div className="flex justify-between">
                    ...
                    <span className="text-xs font-mono text-[var(--navy)] font-bold">{formData.progress}%</span>
                </div>
                <input 
                    type="range" 
                    min="0" 
                    ...
                    className="w-full h-2 bg-neutral-800 rounded-lg appearance-none cursor-pointer accent-[var(--navy)]"
                />
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg-main)]">
                        ...
                        formData.logs.map((log) => (
                            <div key={log.id} className="flex gap-4 group">
                                ...
                                <div className="flex-1 pb-4">
                                    ...
                                    <div className="text-sm text-neutral-300 leading-relaxed bg-[var(--bg-card)] p-3 rounded border border-white/5">
                                        {log.content}
                                    </div>
                ...
                {/* Add Entry */}
                <div className="p-4 bg-[var(--bg-main)] border-t border-white/10 space-y-3">
                    <div className="flex gap-2">
                        <select 
                            value={newLogType}
                            onChange={(e) => setNewLogType(e.target.value as LogType)}
                            className="bg-[var(--bg-card)] border border-white/10 rounded text-[10px] font-bold uppercase text-white p-2 focus:outline-none"
                        >
                        ...
                        <input 
                            ...
                            className="flex-1 bg-[var(--bg-card)] border border-white/10 rounded text-sm text-white px-3 focus:border-[var(--navy)] focus:outline-none"
                        />
                        <button 
                            onClick={handleAddLog}
                            disabled={!newLogContent.trim()}
                            className="px-3 bg-[var(--navy)] hover:bg-[var(--navy)] disabled:bg-neutral-800 text-white rounded transition-colors"
                        >
                            <Plus size={16}/>
                        </button>
                    </div>
